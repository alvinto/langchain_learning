...
class Finding(BaseModel):
    ...
    sub_question: str = Field(description="这条 finding 对应的子问题")
    summary: str = Field(description="子问题的答案，中文，包含具体事实/数字")
    sources: list[Source] = Field(default_factory=list, description="引用的网页列表")
...
def override_or_extend(current: list | None, new: Any) -> list:
    ...
    if isinstance(new, dict) and new.get("__override__"):
        return list(new.get("value", []))
...
class ResearchState(TypedDict, total=False):
    ...
    findings: Annotated[list[Finding], override_or_extend]
    report: str
